...
Map* cargar_generos() {
  ...
  Map *mapaGeneros = map_create(comparar_strings); // clave: nombre del género, valor: List* canciones
  ...
  while ((campos = leer_linea_csv(archivo, ',')) != NULL && k < CancionesACargar){
    ...
    List *lista = NULL;
    MapPair *par = map_search(mapaGeneros, c->genero);
    if (par != NULL) {
      lista = par->value;
    } else {
      lista = list_create();
      map_insert(mapaGeneros, strdup(c->genero), lista);
    }
...
void buscar_por_genero() {
  Map *mapaGeneros = cargar_generos();
